...
| `detector_revision` | SHA-256 identity of that setup revision plus selected universe profile, checks, live settings, and native-indicator calculation version (`detector-sha256:`). It does **not** capture the entire software build, upstream market data, or historical fundamentals. |
| `indicator_calculation_version` | Native-indicator package version plus Edge's Classic calculation-module digest. Populated for new scanner alerts; `null` for old archives or publishers without this evidence. RVOL and other custom measures are not covered by this digest. |
...
| `eventSemantics` | For ORB alerts, `bar-close-cross` means the existing completed-bar trigger; `trade-cross` means a verified fresh Level One trade observation. Do not infer this from the setup name or the current catalog. |
| `trade_cross` | Present only on `trade-cross` ORB alerts: the actual crossing trade price/market time, local receipt/latency, completed opening range identity/level/interval, and stream/quote quality. See [ORB trade-cross contract](ORB_TRADE_CROSS.md). |
...
`source_bar.source` distinguishes `schwab_chart_equity`, `schwab_history_1m`, `schwab_quote_derived`, and Alpaca bars. Quote-derived bars are completed one-minute *estimates* built from Level One quotes (streamed or polled); their intraminute high/low can be less precise than CHART_EQUITY. All labelled feed bars are emitted after their minute closes and carry `complete: true`. Unlabelled or legacy bars use `source: "unknown"` and `complete: null`; no consumer should infer a completed candle from a null. The `id` is a deterministic key for the source, symbol and minute, not a provider event sequence or proof that duplicate/corrected bars cannot occur. On a trade-cross alert it is the latest completed bar used as screening context; the crossing event is `trade_cross.trade_market_timestamp`, which may be in the still-forming next minute.

Edge owns setup detection and the single Schwab market-data stream. Trader consumes this alert and its indicator/trigger evidence; **raw bar series are not sent on the alert WebSocket**. `source_bar` is a compact Edge data reference. When a strategy genuinely needs more bars, it can request `GET /api/bars/{symbol}/1min` from Edge and select the relevant market window using `source_bar.market_timestamp`. This read-only chart/history API uses Schwab REST, not another stream. Its bars can be revised and are not guaranteed to reproduce the exact streamed input identified by `source_bar.id`; the ID is not currently a durable by-ID lookup endpoint. Trader must not recalculate or reconfirm Edge's detector from those bars. Trader may make its own non-streaming calls for separate enrichment, while Alpaca is not a substitute for Schwab live data.
